# Entry point for background remover app
import os
...
def ping_self():
    """تابع پینگ خودکار هر 10 دقیقه"""
    global stop_ping
    
    time.sleep(10)  # کاهش به 10 ثانیه برای تست
    ...
    while not stop_ping:
        try:
            ...
            print(f"✅ پینگ موفق - {datetime.now().strftime('%H:%M:%S')} - Status: {response.status_code}")
            
        except Exception as e:
            print(f"❌ خطا در پینگ: {e}")
        
        # انتظار 5 دقیقه برای تست (300 ثانیه)
        for _ in range(300):
            if stop_ping:
                break
            time.sleep(1)
...
def start_ping_thread():
    """شروع thread پینگ خودکار"""
    global ping_thread, stop_ping
    stop_ping = False
    ping_thread = threading.Thread(target=ping_self)
    ping_thread.daemon = True
    ping_thread.start()
    print("🚀 سیستم پینگ خودکار فعال شد")

def stop_ping_thread():
    """توقف thread پینگ"""
    global stop_ping
    stop_ping = True
    print("🛑 سیستم پینگ خودکار متوقف شد")

# Route برای health check
@app.route('/health')
def health_check():
...
def manual_ping():
    ...
    try:
        ...
        return f"✅ پینگ موفق - Status: {response.status_code}"
    except Exception as e:
        return f"❌ خطا در پینگ: {e}"
